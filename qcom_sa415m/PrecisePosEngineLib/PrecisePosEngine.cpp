...
{
    /*2020-12-23, investigate obs sats lost*/
    /**/
...
void ePProvideGnssSvMeasurement(const epGnssMeasurementReport *msrReport)
{
    ...
    /*
    WangEpoch epoch;
    memset(&epoch, 0x00, EPOCH_SIZE);
    obsqumcom(msrReport, &epoch);
    */
...
void start_correctServ(const char *config)
{
    ...
    LOG_INFO("PPE, loaded Correct LIB\n");
    return ;
}
...
static void ep_init()
{
    if(log_config._rtk_eph_type){
        ...
        subscriptionMsg.subscriptionModMeasurementNHz.constellationType = EP_GNSS_SUB_CONSTELLATION;
        subscriptionMsg.subscriptionModMeasurementNHz.gnssSignal = EP_GNSS_SUB_SIGNAL_TYPE;
        subscriptionMsg.subscriptionModEphemeris.constellationType = EP_GNSS_SUB_CONSTELLATION;
        subscriptionMsg.subscriptionModEphemeris.gnssSignal = EP_GNSS_SUB_SIGNAL_TYPE;
    }else{
        subscriptionMsg.flags = (EP_SUBSCRIPTION_PVT_1HZ_REPORT | \
                             EP_SUBSCRIPTION_MEASUREMENT_1HZ_REPORT | \
                             EP_SUBSCRIPTION_MEASUREMENT_NHZ_REPORT);
    ...
    LOG_INFO("PPE, Subscrible Flags:%d, constellationType:%d, gnssSignal:%d \n", subscriptionMsg.flags, \
             EP_GNSS_SUB_CONSTELLATION, EP_GNSS_SUB_SIGNAL_TYPE);
}
...
{
    ephcom_t ephcom = {0};
    int8_t ret = (int8_t)ppeephcvt(ephemerisReport, &ephcom);
    if(ret  > 0){
