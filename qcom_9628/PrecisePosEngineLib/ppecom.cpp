                      ...
                      ServerEphemeris *ephs)
{
    ...
    for(i=0;i<ephqum->numOfEphemeris;i++){
        ...
        //ephs[index].iodc        = (short)ephqua[i].iodc;//9628 No iodc
        ephs[index].accuracy    = (short)ephqua[i].urai;
        //ephs[index].week        = (unsigned short)(ephqua[i].fullToeSec/604800); //9628 No fullToeSec
        ...
        //ephs[index].ws          = ephqua[i].omegaRad; //9628 No omegaRad
                      ...
                      ServerGloEphemeris *ephs)
{
    ...
    for(i=0;i<ephqum->numOfEphemeris;i++){
        ...
        //N4z                 = ephqua[i].fullToeSec/126230400; /* 1461*86400 */ //9628  No fullToeSec
        //N4r                 = ephqua[i].fullToeSec%126230400;
        ...
        //ephs[i].tog         = ;
        //ephs[i].week        = ;
        //ephs[index].rfchnl      = (short int)(ephqua[i].freqNo-1);//9628 NofreqNo
        ...
        ephs[index].xacc        = ephqua[i].ax; //9628 No lsx,-->ax
                      ...
                      ServerEphemeris *ephs)
{
    ...
    for(i=0;i<ephqum->numOfEphemeris;i++){
        ...
        //ephs[index].week        = (unsigned short)(ephqua[i].fullToeSec/604800)+1024;//9628 No fullToeSec
                      ...
                      ServerEphemeris *ephs)
{
    ...
    for(i=0;i<ephqum->numOfEphemeris;i++){
        ...
        ephs[index].accuracy    = (short)ephqua[i].urai;
        //ephs[index].week        = (unsigned short)(ephqua[i].fullToeSec/604800)+1356;//9628 No fullToeSec
...
//        ephs[index].tgd         = ephqua[i].tgd1Ns*1E-9; //9628 No tgd1Ns
    }
    return index+1;  
}
...
static BYTE obscom(const epGnssMeasurementReport *msrReport, const epGnssSVMeasurementStructType *svobs, BYTE sid, WangEpoch *obss)
{
    ...
    if(!svobs->validityMask.isHealthStatusValid
        ||!svobs->validityMask.isCarrierPhaseValid
        ||svobs->lossOfLock.lossOfContinousLock) return 0;

    switch(svobs->gnssSystem){
        case EP_GNSS_CONSTELLATION_UNKNOWN:
            {
                systemMsec=0.0;
                return 0;
        ...
        case EP_GNSS_CONSTELLATION_GPS:
            {
                if(msrReport->gpsSystemTime.validityMask.isSystemWeekMsecValid){
                    systemMsec=msrReport->gpsSystemTime.systemMsec*1.;//-msrReport->gpsSystemTime.systemClkTimeBias;
                    break;
                }else{
                    return 0;
                }
        ...
        case EP_GNSS_CONSTELLATION_GLONASS:
            {
                if(msrReport->gloSystemTime.validityMask.isGloClkTimeUncMsValid){
                    //systemMsec=msrReport->gloSystemTime.gloMsec*1.+msrReport->gloSystemTime.gloClkTimeBias;
                    systemMsec=msrReport->gloSystemTime.gloMsec*1. ;//- msrReport->gloSystemTime.gloClkTimeBias;
                    break;
                }else{
                    return 0;
        ...
        case EP_GNSS_CONSTELLATION_GALILEO:
            {
                if(msrReport->galSystemTime.validityMask.isSystemWeekMsecValid){
                    //systemMsec=msrReport->galSystemTime.systemMsec*1.+msrReport->galSystemTime.systemClkTimeBias;
                    systemMsec=msrReport->galSystemTime.systemMsec*1. ;//- msrReport->galSystemTime.systemClkTimeBias;
                    break;
                } else{
                    return 0;
        ...
        case EP_GNSS_CONSTELLATION_BEIDOU:
            {
                if(msrReport->bdsSystemTime.validityMask.isSystemWeekMsecValid){
                    //systemMsec=msrReport->bdsSystemTime.systemMsec*1.+msrReport->bdsSystemTime.systemClkTimeBias;
                    systemMsec=msrReport->bdsSystemTime.systemMsec*1. ;//- msrReport->bdsSystemTime.systemClkTimeBias;
                    break;
                }else{ 
                    return 0;
    ...
    switch (sigtype(svobs->gnssSignal)){
        ...
        case SIGL1:
            ...
            //9628 not output pseudorange, calculate
            obss->pdR1[sid]    =(systemMsec-(svobs->svMs*1.+svobs->svSubMs))*0.001*CLIGHT;
...
extern BYTE obsqumcom(const epGnssMeasurementReport *msrReport, ppe_t *ppe)
{
    ...
    for (i=0;i<msrReport->numMeas;i++) {
        ...
        if(log_config._rtk_psr_type){
            ...
            if(!obscom(msrReport, msrReport->svMeasurement+i, j, obss)) continue;
        }
    }
...
static BYTE pvtvalmask(epPVTReportValidity *valbit){
    ...
    //valbit->isFullCovMatrixValid |= 1; //9628 No isFullCovMatrixValid
...
extern BYTE ppeout_rtk(const RTK_result *rtkres,int week,epPVTReport *posrep)
{
    ...
    */
    posrep->gnssSystemTime.gnssSystemTimeSrc=EP_GNSS_CONSTELLATION_GPS;
    posrep->gnssSystemTime.u.gpsSystemTime.validityMask.isSystemWeekValid|=1;
    posrep->gnssSystemTime.u.gpsSystemTime.validityMask.isSystemWeekMsecValid|=1;
    posrep->gnssSystemTime.u.gpsSystemTime.systemWeek=week;
    posrep->gnssSystemTime.u.gpsSystemTime.systemMsec= (uint32_t)(rtkres->dRTK_Resolution_Time*1E3);
...
//    posrep->utcTimestampMs= (timediff(epoch2time(gpst0),epoch2time(utc0))+gtime.time+ gtime.sec)*1E3;
	...
	//if (rtkres->bStatusFlag==4)posrep->posFlags |= EP_POSITION_FLAG_RTK_FIXED;//9628 No 
...
extern uint32_t ephprintep(char *buffer, const epGnssEphemerisReport* ephrep) {
    ...
    char *p=buffer;
    //const epGnssSystemTimeStructType *gpst=&ephrep->gpsSystemTime;//9628 No
...
extern uint32_t obsprintep(char *buffer, BYTE sysidn, const epGnssMeasurementReport *obsrep) {
    ...
    //p+=sprintf(p,"gnssMeasReportType         : %d\n", obsrep->gnssMeasReportType);//9628 No
    p+=sprintf(p,"$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n");
    p+=sprintf(p,"$SYS: %s\n", syschar);
    p+=sprintf(p,"$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n");
...
%15.3f  \n",
                   ...
                   //obss[i].lossOfLock.lossOfContinousLock,  obss[i].cycleSlipCount, obss[i].pseudorange_uncertainty, obss[i].carrierPhase, //9628 No pseudorange_uncertainty 
                   obss[i].lossOfLock.lossOfContinousLock,  obss[i].cycleSlipCount, obss[i].carrierPhase,
